...
<Aside type="caution" title="Immutable expressions only">
PostgreSQL and CockroachDB require immutable functions. AstrolaDB uses `||` instead of `CONCAT()` because `CONCAT()` is not immutable in PostgreSQL.
</Aside>

<Aside type="caution" title="SQLite: no STORED via ALTER TABLE">
SQLite cannot add a STORED generated column via `ALTER TABLE`. Use `.virtual()` instead (VIRTUAL columns are allowed). AstrolaDB returns an error for STORED additions.
</Aside>

<Aside type="caution" title="No self-referencing">
A computed column cannot reference itself or other computed columns.
</Aside>

<Aside type="caution" title="No default values">
Computed columns cannot have a `DEFAULT`.
</Aside>
