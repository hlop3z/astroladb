---
title: Blog Schema
description:
  Complete blog application schema with users, posts, comments, and tags.
---

A full blog schema demonstrating authentication, content management, and
relationships.

## User Table

```js
// schemas/auth/user.js
export default table({
  id: col.id(),
  email: col.email().unique(),
  username: col.username().unique(),
  password: col.password_hash(),
  is_active: col.flag(true),
}).timestamps();
```

## Post Table

```js
// schemas/blog/post.js
export default table({
  id: col.id(),
  author: col.belongs_to("auth.user"),
  title: col.title(),
  slug: col.slug().unique(),
  body: col.body(),
  status: col.enum(["draft", "published", "archived"]).default("draft"),
  published_at: col.datetime().optional(),
})
  .timestamps()
  .soft_delete();
```

## Comment Table

Self-referential replies allow nested comment threads.

```js
// schemas/blog/comment.js
export default table({
  id: col.id(),
  post: col.belongs_to("blog.post"),
  author: col.belongs_to("auth.user"),
  parent: col.belongs_to("blog.comment").optional(),
  body: col.text(),
  is_approved: col.flag(false),
})
  .timestamps()
  .soft_delete();
```

## Tag Table

Many-to-many relationship with posts for flexible categorization.

```js
// schemas/blog/tag.js
export default table({
  id: col.id(),
  name: col.name().unique(),
  slug: col.slug().unique(),
  posts: col.many_to_many("blog.post"),
}).timestamps();
```

## Usage

```js
// Create a user
const user = await db.auth.user.create({
  email: "author@example.com",
  username: "author",
  password: "hashed_password",
});

// Create a post
const post = await db.blog.post.create({
  author: user.id,
  title: "Hello World",
  slug: "hello-world",
  body: "Welcome to my blog!",
  status: "published",
  published_at: new Date(),
});

// Add tags
const tag = await db.blog.tag.create({
  name: "Tutorials",
  slug: "tutorials",
});
await tag.posts.attach(post.id);

// Add a comment with a reply
const comment = await db.blog.comment.create({
  post: post.id,
  author: user.id,
  body: "Great post!",
  is_approved: true,
});

const reply = await db.blog.comment.create({
  post: post.id,
  author: user.id,
  parent: comment.id,
  body: "Thanks!",
  is_approved: true,
});
```
