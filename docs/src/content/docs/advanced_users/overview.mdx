...
| `default`      | `any`     | Default value or expression (e.g. `{"expr": "NOW()"}`) |
...
And because OpenAPI is a global standard that is bigger than any one company,
even if you stop using AstrolaDB, that JSON file can be read by Python scripts,
custom Rust generators, or whatever comes next — **forever**.

> If the band breaks up, you **keep the code**.
