...
// -----------------------------------------------------------------------------
// createDownHookBuilderObject Tests
// -----------------------------------------------------------------------------

func TestSandbox_createDownHookBuilderObject(t *testing.T) {
	t.Run("down_before_hook", func(t *testing.T) {
		sb := NewSandbox(nil)
		sb.BindMigration()
		...
		hooks := sb.GetHooks()
		if len(hooks.DownBefore) != 1 {
			t.Fatalf("Expected 1 down before hook, got %d", len(hooks.DownBefore))
		}
		if hooks.DownBefore[0] != "DROP INDEX idx_test" {
			t.Errorf("DownBefore[0] = %q, want %q", hooks.DownBefore[0], "DROP INDEX idx_test")
		}
	})

	t.Run("down_after_hook", func(t *testing.T) {
		sb := NewSandbox(nil)
		sb.BindMigration()
		...
		hooks := sb.GetHooks()
		if len(hooks.DownAfter) != 1 {
			t.Fatalf("Expected 1 down after hook, got %d", len(hooks.DownAfter))
		}
		if hooks.DownAfter[0] != "VACUUM" {
			t.Errorf("DownAfter[0] = %q, want %q", hooks.DownAfter[0], "VACUUM")
		}
	})

	t.Run("multiple_down_hooks", func(t *testing.T) {
		sb := NewSandbox(nil)
		sb.BindMigration()
		...
		migration({
			...
			down_hook(h) {
				h.before("DELETE FROM logs WHERE table_name = 'users'")
				h.before("DROP INDEX idx_users_email")
				h.after("VACUUM")
				h.after("ANALYZE")
			}
		})
		...
		hooks := sb.GetHooks()
		if len(hooks.DownBefore) != 2 {
			t.Errorf("Expected 2 down before hooks, got %d", len(hooks.DownBefore))
		}
		if len(hooks.DownAfter) != 2 {
			t.Errorf("Expected 2 down after hooks, got %d", len(hooks.DownAfter))
		}
	})
}
...
func TestSandbox_MigrationIntegration(t *testing.T) {
	sb := NewSandbox(nil)
	sb.BindMigration()
	...
	migration({
		...
		renames: {
			"auth.user.verified": "email_verified"
		},
		up(m) {
			m.create_table("auth.user", t => {
				t.id()
		...
		up_hook(h) {
			h.before("CREATE INDEX idx_email ON auth_user(email)")
		},
		down_hook(h) {
			h.before("DROP INDEX idx_email")
			h.after("VACUUM")
		}
	})
	`
	...
	// Verify metadata
	meta := sb.GetMigrationMeta()
	if meta.Description != "Add email verification" {
		t.Errorf("Description = %q, want %q", meta.Description, "Add email verification")
	}

	// Verify hooks
	hooks := sb.GetHooks()
	if len(hooks.Before) != 1 {
		t.Errorf("Expected 1 up before hook, got %d", len(hooks.Before))
	}
	if len(hooks.DownBefore) != 1 {
		t.Errorf("Expected 1 down before hook, got %d", len(hooks.DownBefore))
	}
	if len(hooks.DownAfter) != 1 {
		t.Errorf("Expected 1 down after hook, got %d", len(hooks.DownAfter))
	}
